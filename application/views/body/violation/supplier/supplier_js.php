<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		var dataTable = $('#dt_dtr').DataTable({
			...
			"destroy" : true,
	        "ajax": {  
	            url: "<?php echo site_url('violation/display_supplier_cutoff'); ?>",  
	            type: "POST",
		...
		$('#dt_dtr').on('click', 'a.record', function(){
		    ...
		    $("div.view_company").html('<center><img src="<?php echo base_url('assets/img/gif/loader_seq.gif'); ?>"></center>');
		    $.ajax({
		        type : "POST",
		        ...
		        success : function(data){
		            ...
		            $.ajax({
						type : "POST",	
		                url  : "<?php echo site_url('violation/company_list'); ?>",
	...
	function company_list(cutoff, server, dateFrom, dateTo) {
		...
		var dataTable = $('#dt_sup_violation_list').DataTable({
			...
			stateSave: true,
	        "ajax": {  
	            url: "<?php echo site_url('violation/view_company_list'); ?>",  
	            type: "POST",
		...
		$('#dt_sup_violation_list').off('click', 'a.view_violation').on('click', 'a.view_violation', function(){
	        ...
	        $.ajax({
	            type : "POST",
	            url  : "<?php echo site_url('violation/view_company_violation'); ?>",
	            data : { id:id, dateFrom:dateFrom, dateTo:dateTo, server:server, cutoff:cutoff },
	            success : function(data){
